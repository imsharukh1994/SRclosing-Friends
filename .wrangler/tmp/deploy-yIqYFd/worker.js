...
__name(generateWithModel, "generateWithModel");
...
function simplifyRequests(requests) {
  return requests.slice(0, 300).map((sr) => ({
    id: sr.id,
    subject: sr.subject,
...
__name(simplifyRequests, "simplifyRequests");
async function closurePlan(request, env) {
  ...
  const simplified = simplifyRequests(requests);
